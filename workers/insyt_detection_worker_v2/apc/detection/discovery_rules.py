...
def _rule_from_row(
    ...
    *,
    ...
    if not rule_id:
        rule_id = (
            f"DISCOVERY-{row_number:04d}"
        )

    if not entity_type:
        raise ValueError(
            f"Discovery Detection rule {rule_id} "
...
def _rules_from_workbook(
    ...
    *,
    ...
    requested_sheets = [
        ...
        for sheet_name in (
            sheet_names
            or DEFAULT_DISCOVERY_RULE_SHEETS
        )
        if _clean_text(
            sheet_name
        )
    ...
    if missing_sheets:
        raise ValueError(
            "Discovery Search Rules workbook is missing "
            "requested sheet(s): "
            + ", ".join(
                missing_sheets
            )
    ...
    for sheet_name in requested_sheets:
        ...
        for rule in sheet_rules:
            ...
            if rule_id in rules_by_id:
                raise ValueError(
                    "Duplicate Discovery Search Rule ID "
...
def load_discovery_rules_from_bytes(
...
def load_discovery_rules_from_file(
    path: str | Path,
    *,
    sheet_names: list[str] | tuple[str, ...] | None = None,
) -> list[DetectionRule]:
    ...
    return (
        load_discovery_rules_from_bytes(
            workbook_path.read_bytes(),
            sheet_names=sheet_names,
        )
    )

def discovery_rule_blob_candidates(
    *,
    client: str,
    project: str,
) -> list[str]:
    """
    Ordered Discovery Detection rule workbook locations.
    ...
    routing = AzureRoutingConfig.from_args(
        ...
        review_container=(
            os.getenv(
                "INSYT_REVIEW_CONTAINER_DISCOVERY"
            )
            or None
        ),
    )
    ...
    return [
        (
            ...
            f"{project_key}_Discovery_Search_Rules.xlsx"
        ),
        (
            f"{project_prefix}/source/protocol/"
            "Discovery_Search_Rules.xlsx"
        ...
        (
            "_system/protocol_templates/discovery/"
            "Discovery_Search_Rules.xlsx"
        ),
        (
...
def load_discovery_rules_from_azure(
    *,
    client: str,
    project: str,
    sheet_names: list[str] | tuple[str, ...] | None = None,
    ...
    """
    Load the first available Discovery Detection workbook.
    ...
    if not clean_client:
        raise ValueError(
            "Discovery Detection rules require client."
        )

    if not clean_project:
        raise ValueError(
            "Discovery Detection rules require project."
        )
    ...
    for blob_path in (
        discovery_rule_blob_candidates(
            client=clean_client,
            project=clean_project,
        )
    ):
        ...
        if not rules:
            raise RuntimeError(
                "Discovery Detection rule workbook "
