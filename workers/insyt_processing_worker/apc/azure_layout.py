...
class AzureRoutingConfig:
    ...
    def from_args(
        ...
        return cls(
            ...
            processing_account=processing_account or os.getenv("INSYT_PROCESSING_STORAGE_ACCOUNT", DEFAULT_PROCESSING_ACCOUNT),
            review_account=review_account or os.getenv("INSYT_REVIEW_STORAGE_ACCOUNT", DEFAULT_REVIEW_ACCOUNT),
            processing_container=processing_container or os.getenv("INSYT_PROCESSING_CONTAINER", default_container),
            review_container=review_container or os.getenv("INSYT_REVIEW_CONTAINER", default_container),
    ...
    @property
    def prefix(self) -> str:
        return f"{self.workspace}/{self.client}/{self.project}"

    def processing_blob_url(self, path: str) -> str:
        return f"https://{self.processing_account}.blob.core.windows.net/{self.processing_container}/{path}"

    def review_blob_url(self, path: str) -> str:
        return f"https://{self.review_account}.blob.core.windows.net/{self.review_container}/{path}"
    ...
    def validate(self) -> list[str]:
        ...
        if self.processing_account.lower() != DEFAULT_PROCESSING_ACCOUNT:
            warnings.append(f"processing account is {self.processing_account}, expected {DEFAULT_PROCESSING_ACCOUNT}")
        if self.review_account.lower() != DEFAULT_REVIEW_ACCOUNT:
            warnings.append(f"review account is {self.review_account}, expected {DEFAULT_REVIEW_ACCOUNT}")
        if not self.azure_write:
            warnings.append("azure_write is false; this is a dry-run routing/manifest plan only")
        return warnings


def build_review_promotion_blob_plan(db: LedgerDB, job_id: str, routing: AzureRoutingConfig) -> list[dict[str, object]]:
    rows = db.query(
        """
        SELECT file_id, doc_id, normalized_path, extension, source_bytes, page_count,
               requires_ocr, family_id, parent_file_id, md5, sha1, sha256
...
def build_azure_routing_summary(routing: AzureRoutingConfig, job_id: str | None = None, promotion_count: int = 0) -> dict[str, object]:
    ...
    return {
        ...
        "processing": {
            ...
            "urls": {k: routing.processing_blob_url(v) for k, v in processing_paths.items()},
        },
        "review_outputs": {
            "storage_account": routing.review_account,
            "container": routing.review_container,
            "paths": review_paths,
            "urls": {k: routing.review_blob_url(v) for k, v in review_paths.items()},
        },
        "safety": {
            "processing_account_expected": routing.processing_account.lower() == DEFAULT_PROCESSING_ACCOUNT,
            "review_account_expected": routing.review_account.lower() == DEFAULT_REVIEW_ACCOUNT,
            "accounts_are_separate": routing.processing_account.lower() != routing.review_account.lower(),
...
def export_azure_plan(export_dir: str, job_id: str | None, summary: dict[str, object], promotion_plan: list[dict[str, object]]) -> dict[str, str]:
    out = Path(export_dir)
    out.mkdir(parents=True, exist_ok=True)
    stem = job_id or "azure-routing"
    summary_path = out / f"{stem}.azure_routing_summary.json"
    summary_path.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    paths: dict[str, str] = {"azure_routing_summary_json": str(summary_path)}
    if promotion_plan:
        plan_json = out / f"{stem}.azure_review_promotion_plan.json"
        plan_json.write_text(json.dumps(promotion_plan, indent=2), encoding="utf-8")
        ...
        with plan_csv.open("w", newline="", encoding="utf-8") as f:
            writer = csv.DictWriter(f, fieldnames=list(promotion_plan[0].keys()))
            writer.writeheader()
