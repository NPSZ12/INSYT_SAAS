...
CDS_STORAGE_CONNECTION_STRING = os.getenv(
    "CDS_STORAGE_CONNECTION_STRING"
)

CDS_PROJECTS_CONTAINER = os.getenv(
    "CDS_PROJECTS_CONTAINER",
    "projects",
)
...
def get_blob_service_client() -> BlobServiceClient:
    if not CDS_STORAGE_CONNECTION_STRING:
        raise RuntimeError(
            "Missing CDS_STORAGE_CONNECTION_STRING"
        )
...
def read_project_text_file(
    ...
    if not blob_name.startswith(f"{project_id}/"):
        raise ValueError(
            "Blob path does not belong to requested project"
        )
