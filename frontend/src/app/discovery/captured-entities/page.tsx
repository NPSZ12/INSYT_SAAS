...
type DiscoverydEntitiesResponse = {
  headers: string[];
  rows: Record<string, string>[];
};

function DiscoveryEntitiesPageContent() {
  ...
  const [data, setData] = useState<DiscoverydEntitiesResponse>({
    headers: [],
    rows: [],
  });
  ...
  return (
    <AppShell>
      <PageContainer>
        <PageHeader
          title="discoveryd Entities"
          subtitle={`Protocol-aligned discoveryd entities for ${projectId.replaceAll("_", " ")}${batchId ? ` / ${batchId.replaceAll("_", " ")}` : ""}.`}
        />

        <ContentCard title="discoveryd Entity Table">
          {data.rows.length === 0 ? (
            <p className="text-slate-500">
              No discoveryd entities found for this project/batch.
            </p>
          ) : (
...
export default function DiscoverydEntitiesPage() {
  return (
    <Suspense fallback={<div>Loading Discovery entities...</div>}>
      <DiscoveryEntitiesPageContent />
    </Suspense>
  );
}
