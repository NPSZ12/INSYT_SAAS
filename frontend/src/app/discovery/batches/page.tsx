...
function DiscoveryBatchesPageContent() {
  ...
  async function loadBatches() {
    ...
    try {
      const data = await apiGet(
        `/api/discovery/projects/${selectedProject}/batches`
      );
  ...
  async function createBatch() {
    ...
    try {
      await apiPost(
        `/api/discovery/projects/${selectedProject}/batches/create`,
        {
    ...
    <AppShell>
      <main className="p-8 text-white">
        <h1 className="text-3xl font-bold mb-2">
          INSYT discovery Batches
        </h1>

        <p className="text-slate-400 mb-6">
          Create and manage review batches for the selected discovery project.
        </p>

        {!selectedProject ? (
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 text-slate-400">
            No project selected. Go to Projects and select a discovery project first.
          </div>
        ) : (
          <>
