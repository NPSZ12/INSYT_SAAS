...
const DISCOVERY_SEARCH_RULE_SHEETS = [
...
function DataElementDetectionPageContent() {
  ...
  const [readyData, setReadyData] = useState<DetectionReadyResponse | null>(
    null,
  );

  const [selectedDocIds, setSelectedDocIds] = useState<Set<string>>(new Set());

  const [selectedSearchRuleSheets, setSelectedSearchRuleSheets] = useState<
    Set<string>
  >(new Set(["General Responsiveness"]));

  const [initialLoadingReady, setInitialLoadingReady] = useState(true);

  const [refreshingReady, setRefreshingReady] = useState(false);
  ...
  const [projectImpact, setProjectImpact] = useState<any | null>(null);

  const [loadingProjectImpact, setLoadingProjectImpact] = useState(false);

  const [completedProjectFilters, setCompletedProjectFilters] = useState({
    doc_id: "",
    original_file: "",
    workbook: "",
    sheet: "",
  ...
  const [completedProjectSort, setCompletedProjectSort] = useState<{
  ...
  function toggleCompletedProjectSort(key: CompletedProjectSortKey) {
    setCompletedProjectSort((current) => ({
      key,
      direction:
  ...
  const [readySearchText, setReadySearchText] = useState("");

  const [readyTypeFilter, setReadyTypeFilter] = useState("all");

  type ReadySortKey = "doc_id" | "file" | "type" | "pages" | "text_bytes";

  const [readySort, setReadySort] = useState<{
    key: ReadySortKey;
    direction: "asc" | "desc";
  }>({
    key: "doc_id",
  ...
  const [collapsedReadyJobs, setCollapsedReadyJobs] = useState<
  ...
  const groupedReadyJobs = useMemo(() => {
    const search = readySearchText.trim().toLowerCase();

    const filteredReadyDocs = readyDocs.filter((doc) => {
      const extension = String(doc.extension || "")
        .trim()
        .toLowerCase();

      if (readyTypeFilter !== "all" && extension !== readyTypeFilter) {
        return false;
      }
    ...
    const getSortValue = (doc: DetectionReadyDoc) => {
      switch (readySort.key) {
        case "doc_id":
          return String(doc.doc_id || "");

        case "file":
          return String(doc.original_filename || "");

        case "type":
          return String(doc.extension || "");

        case "pages":
          return Number(doc.page_count || 0);

        case "text_bytes":
          return Number(doc.text_staged_bytes || 0);
    ...
    const sortedReadyDocs = [...filteredReadyDocs].sort((a, b) => {
      ...
      return readySort.direction === "asc" ? comparison : -comparison;
    });

    const groups = new Map<string, DetectionReadyDoc[]>();

    for (const doc of sortedReadyDocs) {
    ...
    const jobDateById = new Map<string, string>();

    for (const job of readyData?.jobs || []) {
      const jobId = String(job.source_job_id || "").trim();

      if (jobId) {
        jobDateById.set(jobId, String(job.completed_at || "").trim());
      }
    }

    return Array.from(groups.entries()).map(([sourceJobId, docs]) => ({
      sourceJobId,
      docs,
      jobDate: jobDateById.get(sourceJobId) || "",
    }));
  }, [readyDocs, readyData?.jobs, readySearchText, readyTypeFilter, readySort]);

  function toggleReadyJob(sourceJobId: string) {
    setCollapsedReadyJobs((current) => ({
      ...current,
  ...
  const filteredCompletedProjectDocs = useMemo(() => {
    const docs: any[] = Array.isArray(projectImpact?.documents)
    ...
    const filtered = docs.filter((doc) => {
      ...
      const elementTypes = Array.from(
        new Set(
          hits
            .map(
              (hit: any) =>
                hit?.entity_type || hit?.category || hit?.type || "",
            )
            .filter(Boolean),
        ),
      ).join(", ");
      ...
      const originalFile =
        doc.original_filename || doc.original_workbook_name || "";

      const detectionJob =
        doc.latest_detection_job_id || doc.detection_job_id || "";

      const detectionDate =
        doc.detected_at || doc.document_index_last_modified || "";

      return (
        contains(doc.doc_id, completedProjectFilters.doc_id) &&
        contains(originalFile, completedProjectFilters.original_file) &&
        contains(
          doc.original_workbook_name,
          completedProjectFilters.workbook,
        ) &&
        contains(doc.sheet_name, completedProjectFilters.sheet) &&
        contains(doc.classification, completedProjectFilters.classification) &&
        contains(hits.length, completedProjectFilters.entities) &&
        contains(elementTypes, completedProjectFilters.elements) &&
        contains(detectionJob, completedProjectFilters.detection_job) &&
        contains(detectionDate, completedProjectFilters.detection_date) &&
        contains(doc.source_job_id, completedProjectFilters.source_job)
      );
    });
    ...
    const getSortValue = (doc: any) => {
      ...
      const elementTypes = Array.from(
        new Set(
          hits
            .map((hit: any) =>
              String(hit.entity_type || hit.category || hit.type || "").trim(),
            )
            .filter(Boolean),
        ),
      ).join(", ");
      ...
      switch (completedProjectSort.key) {
        case "doc_id":
          return String(doc.doc_id || "");
        ...
        case "workbook":
          return String(doc.original_workbook_name || "");

        case "sheet":
          return String(doc.sheet_name || "");

        case "classification":
          return String(doc.classification || "");
        ...
        case "detection_job":
          return String(detectionJob);

        case "detection_date":
          return String(detectionDate);

        case "source_job":
          return String(doc.source_job_id || "");
    ...
    return [...filtered].sort((a, b) => {
      ...
      return completedProjectSort.direction === "asc"
        ? comparison
        : -comparison;
    });
  }, [projectImpact, completedProjectFilters, completedProjectSort]);

  async function loadDetectionReady(isRefresh = false) {
    ...
    try {
      ...
      const response = await apiGet(
        `/api/discovery/processing-center/data-element-detection/ready?${params.toString()}`,
      );

      setReadyData(response);

      const availableDocIds = new Set(
        (response?.docs || []).map((doc: DetectionReadyDoc) => doc.doc_id),
      );
      ...
      console.error("Failed to load detection-ready documents:", err);

      setError(err?.message || "Unable to load Detection Ready documents.");
    } finally {
      setInitialLoadingReady(false);
  ...
  async function loadProjectImpactAssessment() {
    ...
    try {
      ...
      const response = await apiGet(
        `/api/discovery/processing-center/` +
          `data-element-detection/` +
          `project-impact-assessment?` +
          params.toString(),
      );

      setProjectImpact(response);
    } catch (err: any) {
      console.error("Unable to load project Impact Assessment:", err);
    } finally {
  ...
  function selectAllReady() {
    setSelectedDocIds(new Set(readyDocs.map((doc) => doc.doc_id)));
  }
  ...
  function selectAllForJob(docs: DetectionReadyDoc[]) {
    setSelectedDocIds((current) => {
      const next = new Set(current);
  ...
  function clearSelectionForJob(docs: DetectionReadyDoc[]) {
    const jobDocIds = new Set(docs.map((doc) => doc.doc_id));
  ...
  function toggleSearchRuleSheet(sheetName: string) {
    setSelectedSearchRuleSheets((current) => {
      const next = new Set(current);
  ...
  async function startDetectionForJob(
    ...
    const selectedForJob = readyDocs
      .filter(
        (doc) =>
          doc.source_job_id === sourceJobId && selectedDocIds.has(doc.doc_id),
      )
      .map((doc) => doc.doc_id);

    if (!detectAllReady && selectedForJob.length === 0) {
      setError("Select at least one Detection Ready document.");

      return;
    }

    if (selectedSearchRuleSheets.size === 0) {
      setError("Select at least one Discovery Search Rule sheet.");
    ...
    try {
      const response = await apiPost(
        `/api/discovery/processing-center/data-element-detection/start`,
        {
          client: clientId,
          project: projectId,
          ...
          include_phi: false,
          search_rule_sheets: Array.from(selectedSearchRuleSheets),
        },
      );

      const nextDetectionJobId = response?.detection_job_id || "";

      if (!nextDetectionJobId) {
        throw new Error("Detection job was queued without a detection_job_id.");
      }

      setDetectionJobId(nextDetectionJobId);
      setDetectionStatus(response);
    } catch (err: any) {
      console.error("Failed to start Discovery Detection:", err);

      setError(err?.message || "Unable to start Discovery Detection.");
    } finally {
  ...
  useEffect(() => {
    ...
    async function pollDetectionStatus() {
      try {
        ...
        const response = await apiGet(
          `/api/discovery/processing-center/data-element-detection/${encodeURIComponent(
            detectionJobId,
        ...
        if (status === "failed") {
          setError(response?.message || "Discovery Detection failed.");

          return;
        }

        window.setTimeout(pollDetectionStatus, 2000);
      } catch (err: any) {
        if (!cancelled) {
          console.error("Unable to poll detection status:", err);

          window.setTimeout(pollDetectionStatus, 5000);
        }
      }
    }
  ...
  }, [detectionJobId, clientId, projectId]);

  const counts = detectionSummary?.counts || {};

  const documentsScanned =
    counts.documents_scanned ?? detectionStatus?.documents_scanned ?? 0;

  const documentsWithHits =
    counts.documents_with_hits ?? detectionStatus?.documents_with_hits ?? 0;

  const documentsNoHits =
    counts.documents_no_hits ?? detectionStatus?.documents_no_hits ?? 0;

  const documentsNfr =
    counts.documents_nfr ?? detectionStatus?.documents_nfr ?? 0;

  const documentsException =
    counts.documents_exception ?? detectionStatus?.documents_exception ?? 0;

  const completedDetection = documentsScanned + documentsException;

  const hasCurrentDetectionRun = Boolean(
    detectionJobId || detectionStatus?.detection_job_id || detectionSummary,
  );

  const projectCounts = projectImpact?.counts || {};

  const dashboardDocumentsScanned = hasCurrentDetectionRun
    ? documentsScanned
    : Number(projectCounts.documents_completed || 0);

  const dashboardDocumentsWithHits = hasCurrentDetectionRun
    ? documentsWithHits
    : Number(projectCounts.documents_with_hits || 0);

  const dashboardDocumentsNoHits = hasCurrentDetectionRun
    ? documentsNoHits
    : Number(projectCounts.documents_no_hits || 0);

  const dashboardDocumentsNfr = hasCurrentDetectionRun
    ? documentsNfr
    : Number(projectCounts.documents_nfr || 0);

  const dashboardDocumentsException = hasCurrentDetectionRun
    ? documentsException
    : Number(projectCounts.documents_exception || 0);
  ...
  const impactAssessment = detectionSummary?.impact_assessment || {};

  const impactDocumentsWithHits =
    impactAssessment.documents_with_hits_only ?? documentsWithHits;

  const roughNamesWithElements =
    impactAssessment.rough_names_with_attached_elements ?? 0;
  ...
  function exportImpactAssessmentReport() {
    ...
    const url =
      `/api/discovery/processing-center/data-element-detection/` +
    ...
    <div className="min-h-full bg-slate-950 text-slate-100">
      <div className="mx-auto max-w-[1800px] px-6 py-6">
        <div className="mb-6 flex items-start justify-between gap-4">
          <div>
            <div className="mb-2 flex items-center gap-3">
              ...
              <h1 className="text-2xl font-semibold text-white">
                Processing Center - Discovery Detection
              </h1>
            </div>

            <p className="max-w-4xl text-sm leading-6 text-slate-400">
              Scan ingestion-complete documents against the selected Discovery
              responsiveness criteria and project protocol. Review responsive
              hits, no-hit documents, NFR classifications, exceptions, and
              impact assessment totals before documents are promoted for review.
            </p>
          </div>

          <button
            type="button"
            onClick={() => loadDetectionReady(true)}
          ...
          >
            <RefreshCw
              className={`h-4 w-4 ${refreshingReady ? "animate-spin" : ""}`}
            />
            Refresh
          </button>
        </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2 xl:grid-cols-6">
          <MetricCard
            ...
            value={String(readyData?.detection_ready_count || 0)}
          />

          <MetricCard
            label="Documents Scanned"
            value={String(dashboardDocumentsScanned)}
          />

          <MetricCard
            label="Documents With Hits"
            value={String(dashboardDocumentsWithHits)}
          />

          <MetricCard
            label="No Hits"
            value={String(dashboardDocumentsNoHits)}
          />

          <MetricCard label="NFR" value={String(dashboardDocumentsNfr)} />

          <MetricCard
            label="Exceptions"
            value={String(dashboardDocumentsException)}
          />
        </div>
            ...
            <div className="mb-5 rounded-xl border border-slate-800 bg-slate-950/50 p-4">
              <div className="mb-3">
                <div className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-400">
                  Discovery Search Rule Sheets
                </div>

                <div className="mt-1 text-xs text-slate-500">
                  Select one or more Discovery rule sheets to apply to this
                  Detection run.
                </div>
              </div>

              <div className="flex flex-wrap gap-2">
                {DISCOVERY_SEARCH_RULE_SHEETS.map((sheetName) => (
                  <label
                    key={sheetName}
                  ...
                  >
                    <input
                      type="checkbox"
                      checked={selectedSearchRuleSheets.has(sheetName)}
                      onChange={() => toggleSearchRuleSheet(sheetName)}
            ...
            <div className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              <div>
                ...
                <input
                  ...
                  onChange={(event) => setReadySearchText(event.target.value)}
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(event) => setReadyTypeFilter(event.target.value)}
                  className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200"
                >
                  <option value="all">All Types</option>

                  {readyTypeOptions.map((extension) => (
              ...
              <div>
                ...
                <select
                  value={readySort.key}
                  onChange={(event) =>
                    setReadySort((current) => ({
                      ...current,
                      key: event.target.value as ReadySortKey,
                    }))
                  }
                  className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-slate-200"
                >
                  ...
                  <option value="text_bytes">Text Bytes</option>
                </select>
              </div>
            ...
            <div className="max-h-[520px] overflow-y-auto">
              {initialLoadingReady ? (
                ...
                <div className="space-y-5">
                  {groupedReadyJobs.map(({ sourceJobId, docs, jobDate }) => {
                    const collapsed = Boolean(collapsedReadyJobs[sourceJobId]);
                    ...
                    return (
                      ...
                      >
                        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-800 px-4 py-3">
                          <button
                            type="button"
                            onClick={() => toggleReadyJob(sourceJobId)}
                            ...
                            <div>
                              ...
                              <div className="mt-1 flex flex-wrap items-center gap-2">
                                ...
                                <span className="text-xs text-slate-500">
                                  â€¢ {docs.length.toLocaleString()}{" "}
                                  {docs.length === 1 ? "doc" : "docs"}
                                </span>

                                {jobDate ? (
                                  <span className="text-xs text-slate-500">
                                    â€¢ {new Date(jobDate).toLocaleString()}
                                  </span>
                                ) : null}
                              </div>
                            </div>
                          ...
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button
                              type="button"
                              disabled={startingDetection || docs.length === 0}
                              onClick={() =>
                                startDetectionForJob(sourceJobId, false)
                              }
                            ...
                            <button
                              ...
                              onClick={() =>
                                startDetectionForJob(sourceJobId, true)
                              }
        ...
        <CollapsiblePane title="Detection Status" className="mb-6">
          <div className="max-h-[260px] overflow-y-auto p-5">
            {!detectionJobId ? (
              <div className="rounded-xl border border-dashed border-slate-700 px-5 py-8 text-center text-sm text-slate-500">
                No Discovery Detection job started.
              </div>
            ) : (
              <div className="rounded-xl border border-slate-800 bg-slate-950/50 p-4">
                <div className="grid gap-4 md:grid-cols-4">
                  <StatusValue label="Detection Job" value={detectionJobId} />

                  <StatusValue
                    label="Status"
                    value={detectionStatus?.status || "queued"}
                  />

                  <StatusValue
                    label="Stage"
                    value={detectionStatus?.stage || "queued"}
                  />
                ...
                <div className="mt-4 text-sm text-slate-400">
                  {detectionStatus?.message || "Waiting for worker..."}
                </div>
              </div>
            )}
          </div>
        ...
        <CollapsiblePane title="Detection Populations" className="mb-6">
          <div className="grid gap-4 p-5 lg:grid-cols-4">
            ...
            <PopulationPane
              title="Hits"
              description="Documents containing one or more detected Discovery criteria."
              value={documentsWithHits}
            />

            <PopulationPane
              title="No Hits"
              description="Documents scanned with no detected Discovery criteria."
            ...
            <PopulationPane
              ...
              value={documentsNfr + documentsException}
            />
          </div>
        </CollapsiblePane>
        ...
        <CollapsiblePane
          ...
          description="Latest completed Detection set only. Counts and detected Discovery criteria for the documents processed in the current Detection run."
          className="mb-6"
          actions={
            <button
              type="button"
              ...
              <div className="max-h-[420px] overflow-auto rounded-xl border border-slate-800">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 z-10 bg-slate-950 text-left text-xs uppercase tracking-wide text-slate-500">
                    <tr>
                      <th className="px-4 py-3">Discovery Criterion</th>
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {entityTypeCounts.map((row) => (
                      <tr key={row.entity_type} className="bg-slate-950/30">
        ...
        <CollapsiblePane
          ...
          description="Cumulative project assessment of every document that has completed Discovery Detection through the current time. Each Doc ID is counted once using its latest completed Detection result."
          className="mb-6"
          actions={
            <button
              type="button"
              ...
              disabled={
                !projectImpact ||
                Number(projectImpact?.counts?.documents_completed || 0) === 0
              }
              ...
              <>
                <div className="mb-5 grid gap-4 md:grid-cols-3 xl:grid-cols-6">
                  <MetricCard
                    ...
                    value={String(
                      projectImpact?.counts?.documents_completed || 0,
                    )}
                  />
                  ...
                  <MetricCard
                    label="Hits"
                    value={String(
                      projectImpact?.counts?.documents_with_hits || 0,
                    )}
                  />
                  ...
                  <MetricCard
                    label="No Hits"
                    value={String(
                      projectImpact?.counts?.documents_no_hits || 0,
                    )}
                  />

                  <MetricCard
                    label="NFR"
                    value={String(projectImpact?.counts?.documents_nfr || 0)}
                  />

                  <MetricCard
                    label="Exceptions"
                    value={String(
                      projectImpact?.counts?.documents_exception || 0,
                    )}
                  />

                  <MetricCard
                    label="Elements Identified"
                    value={String(projectImpact?.counts?.entity_hit_count || 0)}
                  />
                </div>

                {(projectImpact?.entity_type_counts || []).length === 0 ? (
                  <div className="rounded-xl border border-dashed border-slate-700 px-5 py-10 text-center text-sm text-slate-500">
                  ...
                  <div className="max-h-[420px] overflow-auto rounded-xl border border-slate-800">
                    <table className="min-w-full text-sm">
                      ...
                      <tbody className="divide-y divide-slate-800">
                        {(projectImpact?.entity_type_counts || []).map(
                          (row: any) => (
        ...
        <CollapsiblePane
          ...
          description="Every document in this project with a completed Discovery Detection result, using the latest completed Detection state per Doc ID."
          className="mb-6"
        >
          <div className="p-5">
            {loadingProjectImpact ? (
              ...
              <div className="max-h-[420px] overflow-auto rounded-xl border border-slate-800">
                <table className="min-w-full text-sm">
                  <thead className="sticky top-0 z-10 bg-slate-950 text-left text-xs uppercase tracking-wide text-slate-500">
                    <tr>
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() => toggleCompletedProjectSort("doc_id")}
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Doc ID{" "}
                          {completedProjectSort.key === "doc_id"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                        </button>
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() =>
                            toggleCompletedProjectSort("original_file")
                          }
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Original File{" "}
                          {completedProjectSort.key === "original_file"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() => toggleCompletedProjectSort("workbook")}
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Workbook{" "}
                          {completedProjectSort.key === "workbook"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() => toggleCompletedProjectSort("sheet")}
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Sheet{" "}
                          {completedProjectSort.key === "sheet"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() =>
                            toggleCompletedProjectSort("classification")
                          }
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Classification{" "}
                          {completedProjectSort.key === "classification"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3 text-right">
                        <button
                          type="button"
                          onClick={() => toggleCompletedProjectSort("entities")}
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Entities{" "}
                          {completedProjectSort.key === "entities"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() => toggleCompletedProjectSort("elements")}
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Detected Discovery Criteria{" "}
                          {completedProjectSort.key === "elements"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() =>
                            toggleCompletedProjectSort("detection_job")
                          }
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Detection Job{" "}
                          {completedProjectSort.key === "detection_job"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() =>
                            toggleCompletedProjectSort("detection_date")
                          }
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Detection Date{" "}
                          {completedProjectSort.key === "detection_date"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                      ...
                      <th className="px-4 py-3">
                        <button
                          type="button"
                          onClick={() =>
                            toggleCompletedProjectSort("source_job")
                          }
                          className="whitespace-nowrap hover:text-slate-200"
                        >
                          Source Job{" "}
                          {completedProjectSort.key === "source_job"
                            ? completedProjectSort.direction === "asc"
                              ? "â†‘"
                              : "â†“"
                            : "â†•"}
                    ...
                    <tr className="border-t border-slate-800 bg-slate-950">
                      {[
                        ...
                        <th key={field} className="px-2 pb-3">
                          <input
                            type="text"
                            value={
                            ...
                            onChange={(event) =>
                              setCompletedProjectFilters((current) => ({
                                ...current,
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {filteredCompletedProjectDocs.map((doc: any) => {
                      const hits = Array.isArray(doc?.hits) ? doc.hits : [];
...
function PopulationPane({
  ...
  return (
    <div className="min-h-[180px] rounded-xl border border-slate-800 bg-slate-950/50 p-4">
      ...
      <div className="mt-6 text-3xl font-semibold text-slate-300">{value}</div>
    </div>
  );
}
