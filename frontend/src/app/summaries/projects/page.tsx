...
export default function ProjectsPage() {
  ...
  function userCanAccessWorkspace(currentUser: StoredUser | null) {
    ...
    return allowedWorkspaces.includes("summaries");
  }
  ...
  function loadProjects(client: string) {
    ...
    apiGet(
      `/api/summaries/clients/${encodeURIComponent(
        client
      )}/projects`
    )
      .then((response) => {
        ...
        setProjects(
          visibleProjects.map((projectId: string) => ({
            ...
            status: "Active",
            progress: 0,
            openHref: `/summaries/project-dashboard?client=${encodeURIComponent(
              client
              ...
              {projects.map((project) => (
                <ProjectCard
                  ...
                  onOpen={() => {
                    ...
                    router.push(
                      `/summaries/project-dashboard?client=${encodeURIComponent(
                        selectedClient
                      )}&project=${encodeURIComponent(projectId)}`
                    );
                  }}
